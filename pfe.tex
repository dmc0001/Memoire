...
\vspace{1em}
\noindent\textbf{Keywords:} Intrusion Detection Systems (IDS), Denial-of-Service (DoS), Reinforcement learning (RL), CSE-CIC-IDS2018, Cybersecurity, Network Traffic Analysis.
...
\begin{otherlanguage}{arabic}
    ...
    \begin{quote}
        ...
        في ضوء تزايد انتشار هجمات الحرمان من الخدمة، أصبحت آليات الكشف القوية ضرورية لضمان توفر وموثوقية شبكات الحاسوب. تقترح هذه الأطروحة نهجًا جديدًا للكشف عن التسلل يستفيد من التعلم المعزز لتحديد وتخفيف هجمات الحرمان من الخدمة. يتعلم وكيل التعلم المعزز سياسات الكشف المثلى من خلال التفاعل مع بيئة شبكة محاكاة، مما يحسن أداءه باستمرار من خلال التغذية الراجعة القائمة على المكافآت. يتم استخراج ميزات حركة مرور الشبكة الرئيسية واستخدامها كمدخلات حالة للوكيل، مما يمكنه من التمييز بين النشاط الحميد والخبيث. يتم تقييم النموذج في مجموعات البيانات المرجعية مثل CSE-CIC-IDS2018، مما يدل على دقة كشف عالية، ومعدلات إيجابية كاذبة منخفضة، والقدرة على التكيف مع أنماط الهجوم المتطورة.
        % --- END OF ARABIC ABSTRACT TEXT ---
    \end{quote}

    \vspace{1em}
    \noindent\textbf{الكلمات المفتاحية:} أنظمة كشف التسلل (IDS)، الحرمان من الخدمة (DoS)، التعلم المعزز (RL)، CSE-CIC-IDS2018، الأمن السيبراني، تحليل حركة مرور الشبكة.
\end{otherlanguage}
...
\begin{otherlanguage}{french}
    ...
    \begin{quote}
        \large
        Face à la prévalence croissante des attaques par déni de service (DoS), des mécanismes de détection robustes sont devenus essentiels pour garantir la disponibilité et la fiabilité des réseaux informatiques. Cette thèse propose une nouvelle approche de détection d'intrusion qui s'appuie sur l'apprentissage par renforcement (RL) pour identifier et atténuer les attaques DoS. L'agent RL apprend des politiques de détection optimales en interagissant avec un environnement réseau simulé, améliorant continuellement ses performances grâce à des retours basés sur des récompenses. Les principales caractéristiques du trafic réseau sont extraites et utilisées comme entrées d'état pour l'agent, lui permettant de distinguer l'activité bénigne de l'activité malveillante. Le modèle est évalué sur des ensembles de données de référence tels que CSE-CIC-IDS2018, démontrant une grande précision de détection, de faibles taux de faux positifs et une adaptabilité aux modèles d'attaque en évolution. Comparé aux classificateurs d'apprentissage automatique traditionnels, le système basé sur le RL montre des capacités de réponse et de prise de décision améliorées dans des conditions de réseau dynamiques. Les recherches futures exploreront l'intégration de ce système dans des infrastructures réseau en temps réel et l'amélioration de son extensibilité pour une détection plus large des menaces.
    \end{quote}

    \vspace{1em}
    \noindent\textbf{Mots-clés :} Systèmes de Détection d'Intrusion (IDS), Déni de Service (DoS), Apprentissage par Renforcement (RL), CSE-CIC-IDS2018, Cybersécurité, Analyse du Trafic Réseau.
\end{otherlanguage}
